...
type GreeterClient interface {
	...
	// 双向流式数据
	BidHello(ctx context.Context, opts ...grpc.CallOption) (Greeter_BidHelloClient, error)
	// 传送 header trailer
...
func (c *greeterClient) BidHello(ctx context.Context, opts ...grpc.CallOption) (Greeter_BidHelloClient, error) {
	stream, err := c.cc.NewStream(ctx, &Greeter_ServiceDesc.Streams[2], "/pb.Greeter/BidHello", opts...)
	if err != nil {
		return nil, err
	}
	x := &greeterBidHelloClient{stream}
	return x, nil
}

type Greeter_BidHelloClient interface {
	Send(*HelloRequest) error
	Recv() (*HelloResponse, error)
	grpc.ClientStream
}

type greeterBidHelloClient struct {
	grpc.ClientStream
}

func (x *greeterBidHelloClient) Send(m *HelloRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *greeterBidHelloClient) Recv() (*HelloResponse, error) {
	m := new(HelloResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type GreeterServer interface {
	...
	// 双向流式数据
	BidHello(Greeter_BidHelloServer) error
	// 传送 header trailer
...
func (UnimplementedGreeterServer) BidHello(Greeter_BidHelloServer) error {
	return status.Errorf(codes.Unimplemented, "method BidHello not implemented")
}
...
func _Greeter_BidHello_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreeterServer).BidHello(&greeterBidHelloServer{stream})
}

type Greeter_BidHelloServer interface {
	Send(*HelloResponse) error
	Recv() (*HelloRequest, error)
	grpc.ServerStream
}

type greeterBidHelloServer struct {
	grpc.ServerStream
}

func (x *greeterBidHelloServer) Send(m *HelloResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *greeterBidHelloServer) Recv() (*HelloRequest, error) {
	m := new(HelloRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var Greeter_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "BidHello",
			Handler:       _Greeter_BidHello_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
